...
namespace {
  // Portable fnmatch wrapper
  bool pattern_match(const string& pattern, const string& str) {
#ifdef _WIN32
    return PathMatchSpecA(str.c_str(), pattern.c_str()) != 0;
#else
    ...
    if ((notify_element = event_element->FindElement("notify")) != 0) {
      ...
      while (check_element) {
        ...
        if (cs.Property.empty()) {
          cerr << check_element->ReadFrom()
               << fgred << "  <check> element requires a 'property' attribute." << reset << endl;
          delete newEvent;
          return false;
        }
        if (!check_element->GetAttributeValue("value").empty()) {
          ...
          cerr << check_element->ReadFrom()
               << fgred << "  <check> element requires a 'value' attribute." << reset << endl;
          delete newEvent;
          return false;
        }
        ...
        if (!thisEvent.CheckSpecs.empty()) {
          ...
          for (unsigned int cs = 0; cs < thisEvent.CheckSpecs.size(); cs++) {
            ...
            if (PropertyManager->HasNode(chk.Property)) {
              ...
              cerr << fgred << "  CHECK ERROR: Property '" << chk.Property
                   << "' not found in property tree." << reset << endl;
              eventCheckFails++;
              CheckFailCount++;
              continue;
            }
            ...
            if (passed) {
              cout << "    CHECK PASS: " << chk.DisplayString
                   << " = " << actualValue
                   << " expected " << chk.ExpectedValue
                   << " tol " << effectiveTol;
              if (!chk.Message.empty()) cout << " : " << chk.Message;
              cout << endl;
              CheckPassCount++;
            } else {
              cout << fgred << "    CHECK FAIL: " << chk.DisplayString
                   << " = " << actualValue
                   << " expected " << chk.ExpectedValue
                   << " tol " << effectiveTol;
              if (!chk.Message.empty()) cout << " : " << chk.Message;
          ...
          if (eventCheckFails == 0) {
            cout << "  EVENT PASS: " << thisEvent.Name
            ...
            if (eventFailed) {
              cout << fgred << "  EVENT FAIL: " << thisEvent.Name
                   << " (" << eventCheckFails << " of " << eventCheckTotal
                   << " checks failed)" << reset << endl;

              if (thisEvent.HaltOnFailure) {
                cout << fgred << "  HALT: halt-on-failure is set, terminating simulation."
                     << reset << endl;
                return false;
              }
            } else {
...
bool FGScript::LoadTolerances(const SGPath& path)
{
  ...
  if (!document) {
    cerr << "Tolerances file could not be loaded: " << path << endl;
    return false;
  }

  cout << "Tolerances loaded from: " << path << endl;
...
void FGScript::PrintCheckSummary(void) const
{
  ...
  cout << endl;
  cout << "=====================================================================" << endl;
  cout << "  CHECK SUMMARY: " << CheckPassCount << " passed, "
       << CheckFailCount << " failed, " << total << " total" << endl;
  cout << "=====================================================================" << endl;
}
